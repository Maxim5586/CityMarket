...
const vetement = mongoose.model('vetement', new mongoose.Schema({
  vetement_id: String,
  nom_vetement: String,
...
const ItemCommSchema = mongoose.model('item_comm', new mongoose.Schema({
  vetement_id: Number,
  nom_vetement: String,
  type_vetement: String,
  prix: Number,
...
app.post('/oublier-mdp', (req, res) => {
  ...
  ` + link + `<br> <h3> Ce lien expirera dans 15 minutes.</h3>`;
  ...
  async function changerMDP(password2, email2) {
   ...
   bcrypt
    ...
    try {
      ...
      const updatedUser = await clientSchema.findOneAndUpdate(filter, update, options);
...
app.post('/articles/filter', async (req, res) => {
  try {
    const { searchTerm, selectedBrand, selectedSize, sortOrderValue } = req.body;
...
app.get('/articles/:vetement_id', async (req, res) => {
  try {
    ...
    const titre = vetementSelectionnee.marque_vetement + " - " + vetementSelectionnee.nom_vetement;
...
app.post('/connecter-utilisateur', function (req, res) {
  ...
  clientSchema.findOne({ courriel: email }).then((user) => {
    ...
    if (user && validate) {
      ...
      alert("Le courriel ou le mot de passe que vous avez entrez est invalide");
      res.end;
    }
 ...
 bcrypt
  ...
  if(password != password2){
    ...
    alert("Vous n'avez pas entrez un numero de telephone valide");;
      ...
      setTimeout(function() {
            clientSchema.findOne({ courriel: email })
        .then(user => {
          if (user) {
            logger.info('Utilisateur existe:', user);
            alert("Le courriel que vous avez entrez est deja relié a un compte");
            res.redirect('/inscription');
          } else {
...
async function main() {
  ...
  if (mongoose.models.item_comm) {
    ...
    userSchema = mongoose.model('item_comm', new mongoose.Schema({
      vetement_id: Number,
      nom_vetement: String,
      type_vetement: String,
      prix: Number,
...
app.post('/ajoutVetement',upload.single('image'), async (req, res) => {
  ...
  if (!user  || user.taille_vetement != taille) {
    ...
    logger.info(
      "L'article que vous essayer d'ajouter existe deja"
    );
